...
class Wav2Vec2FeedForward(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        if self.train_adapters:
            ...
            if self.train_adapters_blocks:
                ...
                self.is_meta_adapter = (
                    True if isinstance(adapter_config, MetaAdapterConfig) else False
                )
    ...
    def forward(
        ...
        if self.train_adapters and self.train_adapters_blocks:
            hidden_states = self.adapter_controller(
                task if not self.is_meta_adapter else task_embedding, hidden_states
            )
...
class Wav2Vec2EncoderStableLayerNorm(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        if self.train_adapters:
            self.unique_hyper_net = (
                isinstance(adapter_config, MetaAdapterConfig)
                and adapter_config.unique_hyper_net
            )
            self.efficient_unique_hyper_net = (
                isinstance(adapter_config, MetaAdapterConfig)
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            wav2vec2_block_adapters = None
            if self.train_adapters and (
                self.unique_hyper_net or self.efficient_unique_hyper_net
            ):
